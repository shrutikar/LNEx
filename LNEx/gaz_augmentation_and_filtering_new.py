...
def filter_geo_locations(geolocation):
    ''' Filters out the gazetteer location names.

    input>  type(geolocation): string

    output> type(all_geolocations): list'''
    ...
    original_text = geolocation

    geolocation = geolocation.replace("( ", "(").replace(" )", ")")

    geolocation = geolocation.lower()

    # remove bracketed blacklisted bracketed mentions
    for name_to_remove in names_to_remove:
        if name_to_remove in geolocation:
            geolocation = geolocation.replace(name_to_remove, "")

    # punctuation padding
    geolocation = re.sub('([,;])', r'\1 ', geolocation)
    geolocation = re.sub('\s{2,}', ' ', geolocation)

    names = preprocess_name(geolocation)

    ###################################

    all_geolocations = [geolocation]
    ...
    for name in names:
        ...
        # prevents collisions
        if name not in all_geolocations:
            all_geolocations.append(name)

    return all_geolocations

################################################################################

def augment(geolocation):
    '''Augments the location names using skip grams'''

    # augmentation includes filtering
    all_geolocations = filter_geo_locations(geolocation)
        ...
        if name not in list_tops_to_remove and len(
            ...
            if alphanumeric_name != name and alphanumeric_name not in gaz_stopwords:
                ...
                if alphanumeric_name not in lns:
                    all_geolocations.append(alphanumeric_name)

    # step 3 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

    # create skip grams

    lns2 = set(all_geolocations)
        ...
        for new_name in set(flexi_grams):
            ...
            if len(nospaces) > 2 and not nospaces.isdigit():
                ...
                if new_name not in lns:
                    all_geolocations.append(new_name)

    return all_geolocations, get_extended_words3(all_geolocations)
